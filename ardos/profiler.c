...
void tickTask()
{
 ...
 void updateWorstTimeAnalysis(int PID)
 {
	...
	if (value < _profileBin[PID]) {
		...
		uint8_t lowByte = ((_profileBin[PID] >> 0) & 0xFF);
		uint8_t highByte = ((_profileBin[PID] >> 8) & 0xFF);
		eeprom_write_byte(startingAddress, lowByte);
		eeprom_write_byte(startingAddress + 1, highByte);
...
uint16_t getTaskProfileFromROM(int PID)
 {
	...
	uint8_t lowByte = eeprom_read_byte(startingAddress);
    uint8_t highByte = eeprom_read_byte(startingAddress + 1);
